# Drawing my own plot for region 30
# https://sinarueeger.github.io/post/locuszoomplot/

myplot <- function(sel.snp, sel.chr, sel.pos, sel.rsid, range, data.region, filename, cred.set) {
  library(ggplot2)
  library(biomaRt) 
  library(patchwork)
  library(dplyr)
  ...
  p1 <- ggplot(data.region) + 
    geom_point(aes(POS, -log10(pval.x)), shape=1) + 
    labs(title = "Locuszoomplot for GWAS T2D", subtitle = paste("Summary statistics for chromosome", sel.chr, "from", format((sel.pos - range), big.mark = "'"), "to", format((sel.pos + range), big.mark = "'"), "bp"))
  
  snp.ensembl.grch37 <- useMart(host='http://grch37.ensembl.org', 
                                biomart='ENSEMBL_MART_SNP', 
                                dataset='hsapiens_snp')
  # snp.ensembl.grch37 <- useEnsembl(biomart = "snp", dataset = "hsapiens_snp", GRCh = 37)
  out.bm.grch37 <- getBM(
  ...
  out.bm.gene <- getBM(attributes = c('external_gene_name'), 
                       ...
                       values = unique(out.bm.grch37$ensembl_gene_stable_id), 
                       mart = gene.ensembl)
  ...
  out.bm.genes.region <- out.bm.genes.region %>% 
    mutate(gene_biotype_fac=fct_relevel(as.factor(gene_biotype),"protein_coding"), 
           external_gene_name = fct_reorder2(external_gene_name, start_position, gene_biotype_fac))
  
  ## plot genes
  p2 <- ggplot(out.bm.genes.region) + 
  ...
  #png(width = (160 / 25.4), height = (plot.height / 25.4), filename = filename)
  p3 <- p1b + p2 + plot_layout(ncol = 1, heights = c(6, 6))
  print(p3)
  #dev.off()
}
