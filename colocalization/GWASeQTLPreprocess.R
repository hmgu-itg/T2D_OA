...
###################################################
read_eQTL_eGenes <- function(tissue, source, dict_eQTL, dict_eGenes, delete.indels=FALSE){
  ##################################
  ...
  if (source == "FunGen") {
    if (!file.exists(paste0("/storage/hmgu/projects/OA_T2D/processed_data/eQTL_", tissue, ".csv"))) {
      print("FunGen preprocessed file does not exist")
      tmp <- fread(paste("/storage/hmgu/projects/OA_T2D/data_original/FunGen_eQTL_", tissue, ".FastQTL_perm_nom_info.ForMSK-KP_16Jan2021.txt.gz", 
                         sep=""), verbose=FALSE)
      
      dict_eQTL[[tissue]] <- tmp[, .(CHR=as.integer(sub(":.*", "", genotype_id)), 
      ...
      dict_eGenes[[tissue]] <- tmp[eGene==1, .(CHR=as.integer(sub(":.*", "", genotype_id)), 
                                               POS=as.integer(sub(".*:", "", genotype_id)),
                                               geneID=sub(".*_", "", phenotype_id))]
      
      write_output(tissue, dict_eQTL, paste0("/project_data/processed_data/eQTL_", tissue, ".csv"))
      write_output(tissue, dict_eGenes, paste0("/project_data/processed_data/eGenes_", tissue, ".csv"))
    }
    
    else{
      print("FunGen preprocessed file already exists")
      dict_eQTL[[tissue]] <- fread(paste0("/storage/hmgu/projects/OA_T2D/processed_data/eQTL_", tissue, ".csv"), verbose=FALSE)
      dict_eGenes[[tissue]] <- fread(paste0("/storage/hmgu/projects/OA_T2D/processed_data/eGenes_", tissue, ".csv"), verbose=FALSE)
    }
  }
  
  else if (source == "GTEx"){
    if (!file.exists(paste0("/storage/hmgu/projects/OA_T2D/processed_data/eQTL_", tissue, ".csv"))) {
      print("GTEx preprocessed file does not exist")
      # tmp_eQTL <- fread(paste("/storage/hmgu/projects/OA_T2D/data_original/", tissue, ".v7.signif_variant_gene_pairs.txt.gz", sep=""), verbose=FALSE)
      tmp_eQTL <- fread(paste("/storage/hmgu/projects/OA_T2D/data_original/", tissue, ".allpairs.txt.gz", sep=""), verbose=FALSE)
      tmp_eGenes <- fread(paste("/storage/hmgu/projects/OA_T2D/data_original/", tissue, ".v7.egenes.txt.gz", sep=""), verbose=FALSE)
      
      tmp <- merge(tmp_eQTL, tmp_eGenes, by="gene_id")
      
      # dict_eQTL[[tissue]] <- tmp[, .(CHR=ifelse(sub("_.*", "", variant_id.x)=="X", 23L, as.integer(sub("_.*", "", variant_id.x))), 
      #                                POS=as.integer(gsub(".*_(\\d+)_.*", "\\1", variant_id.x)), 
      #                                geneID=sub("\\..*", "",gene_id), pval=pval_nominal.x, beta=slope.x, se=slope_se.x, MAF=maf.x,
      #                                N=num_var, EA=alt, NEA=ref)]
      dict_eQTL[[tissue]] <- tmp[, .(CHR=sub("_.*", "", variant_id.x), 
                                     POS=as.integer(gsub(".*_(\\d+)_.*", "\\1", variant_id.x)), rsID=rs_id_dbSNP147_GRCh37p13,
                                     geneID=sub("\\..*", "",gene_id), pval=pval_nominal.x, beta=slope.x, se=slope_se.x, MAF=maf.x,
                                     N=num_var, EA=alt, NEA=ref)] %>% .[CHR!="X", ] %>% .[, CHR:=as.integer(CHR)]
      
      dict_eGenes[[tissue]] <- tmp_eGenes[, .(CHR=ifelse(gene_chr=="X", 23L, as.integer(gene_chr)), # or chr?
                                              POS=as.integer(gene_start), # or pos?
                                              geneID=sub("\\..*", "",gene_id))]
      if (delete.indels){
        dict_eQTL[[tissue]] <- dict_eQTL[[tissue]][nchar(EA)==1 & nchar(NEA)==1]
      }
      write_output(tissue, dict_eQTL, paste0("/project_data/processed_data/eQTL_", tissue, ".csv"))
      write_output(tissue, dict_eGenes, paste0("/project_data/processed_data/eGenes_", tissue, ".csv"))
    }
      ...
      print("GTEx preprocessed file already exists")
      dict_eQTL[[tissue]] <- fread(paste0("/storage/hmgu/projects/OA_T2D/processed_data/eQTL_", tissue, ".csv"), verbose=FALSE)
      dict_eGenes[[tissue]] <- fread(paste0("/storage/hmgu/projects/OA_T2D/processed_data/eGenes_", tissue, ".csv"), verbose=FALSE)
    }
  }
    ...
    if (!file.exists(paste0("/storage/hmgu/projects/OA_T2D/processed_data/eQTL_", tissue, ".csv"))) {
      ...
      tmp <- fread(paste("/storage/hmgu/projects/OA_T2D/data_original/InsPIRE_", tissue, "_Gene_eQTLs_Nominal_Pvalues.txt.gz", 
                         sep=""), verbose=FALSE)
      # tmp_eGenes <- fread(paste("/storage/hmgu/projects/OA_T2D/data_original/", tissue, "_independent_gene_eQTLs.txt", sep=""), verbose=FALSE)
      
      col_names <- colnames(tmp)[-1]
      tmp <- tmp[, !"FreqALT"]
      setnames(tmp, col_names)
      
      dict_eQTL[[tissue]] <- tmp[, .(CHR=SNPchr, POS=SNPposition, rsID=rsID, geneID=sub("\\..*", "",GeneID), 
                                     pval=Pvalue, beta=Slope, se=SE, MAF=ifelse(FreqREF<FreqALT, FreqREF, FreqALT), 
                                     N=NumSNPtest, EA=ALT, NEA=REF)]
      # dict_eGenes[[tissue]] <- unique(tmp_eGenes, by="GeneID")[, .(CHR=ChrPheno, 
      #                                                              POS=as.integer(StartPheno),
      #                                                              geneID=sub("\\..*", "",GeneID))]
      # delete "MERGED"
      dict_eQTL[[tissue]] <- dict_eQTL[[tissue]][!rsID %like% "MERGED"] %>% .[!rsID %like% "rs", rsID:="."]
      
      if (delete.indels){
        dict_eQTL[[tissue]] <- dict_eQTL[[tissue]][!(EA %in% c("D", "I"))]
        dict_eQTL[[tissue]] <- dict_eQTL[[tissue]][nchar(EA)==1 & nchar(NEA)==1]
      }
  ...
  # read GWAS summary statistics
    ...
    if (source == "GO") {
      ...
      if (!file.exists(paste0("/project_data/processed_data/GWAS_", trait, ".csv"))) {
        ...
        tmp <- fread(paste("/storage/hmgu/projects/OA_T2D/data_original/Mahajan.NatGenet2018b.", trait, ".European.txt.gz", sep=""), 
                     verbose=FALSE)
...
read_TSS <- function(tissue, source, dict, dict_eGenes){
  if (source=="FunGen"){
    dict[[tissue]] <- fread(paste("/storage/hmgu/projects/OA_T2D/data_original/TSS_FunGen.txt", sep=""), 
                            verbose=FALSE)[, .(CHR=`#Chr`, START=start, ID=sub(".*_", "", ID))]
  }
  else if (source=="GTEx"){
    dict[[tissue]] <- dict_eGenes[[tissue]][, .(CHR=CHR, START=POS, ID=geneID)]
  }
  else if (source=="InsPIRE"){
    dict[[tissue]] <- dict_eGenes[[tissue]][, .(CHR=CHR, START=POS, ID=geneID)]
  }
...
select_eGenes <- function(tissue, dict_signals, dict_eGenes, wd=100000) {
  eGenes <- data.table()
  for (i in 1:nrow(dict_signals)){
    eGenes <- rbind(eGenes, dict_eGenes[[tissue]][CHR %in% dict_signals$CHR[i] &
                                                    POS %between% .(dict_signals$POS[i]-wd, dict_signals$POS[i]+wd)])
  }
  dict_eGenes[[paste(tissue, "signals", sep="_")]] <- eGenes[, unique(geneID)]
}

select_eQTL <- function(tissue, dict_eGenes, dict_eQTL) {
  key <- paste(tissue, "signals", sep="_")
  dict_eQTL[[key]] <- rbind(dict_eQTL[[key]], dict_eQTL[[tissue]][geneID %in% dict_eGenes[[key]]])
}

select_eQTL2 <- function(tissue, dict_eQTL, credset.id) {
  dict_eQTL[[paste(tissue, "coloc", sep="_")]] <- dict_eQTL[[tissue]][ID %in% credset.id]
}
    ...
    if (length(IDlist[[id]])>1){
      ...
      lapply(IDlist[[id]][-1], function(i){
        ...
        if (altEA != refEA) {
          ...
          if(has.key(i, dict_eQTL)) dict_eQTL[[i]][ID==id, beta:=-beta]
...
TISSUE <- data.table(tissue=c("HighGradeCartilage", "LowGradeCartilage",
                              "Synovium", "PancreaticIslets"),
                     source=c(rep(c("FunGen"),3), "InsPIRE"))
...
TISSUE[, read_eQTL_eGenes(tissue=tissue, dict_eQTL=eQTL, dict_eGenes=eGenes, source=source, delete.indels=TRUE), by=tissue]
